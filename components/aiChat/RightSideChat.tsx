...
const MarkdownComponents = {
  ...
  // Headings
  h1: ({ children }: any) => (
    <h1 className="text-2xl font-bold mb-4 mt-6 pb-2 border-b dark:border-slate-700">
      {children}
    </h1>
  ),
  h2: ({ children }: any) => (
    <h2 className="text-xl font-bold mb-3 mt-5 pb-1 border-b dark:border-slate-700/50">
      {children}
    </h2>
  ),
  h3: ({ children }: any) => (
...
const ChatMessage = memo(({ message }: { message: any }) => {
  ...
  return (
    ...
    >
      <div
        className={`flex gap-3 max-w-[90%] md:max-w-[85%] ${isUser ? "flex-row-reverse" : "flex-row"}`}
      >
        {/* Avatar */}
        <div
        ...
        >
          {message.isLoading ? (
            ...
            <div
              className={`markdown-body ${isUser ? "text-white" : "dark:text-slate-100"}`}
            >
              {/* 
...
const RightSideChat = ({
  ...
  return (
    <div className="flex flex-col h-full bg-white rounded-xl overflow-hidden shadow-sm border border-gray-200  ">
      <div className="p-4   flex justify-between items-center bg-white/50 0 backdrop-blur z-10 border-gray-200  border-b ">
        <h2 className="font-bold flex items-center gap-2 text-slate-800 dark:text-slate-100">
          <LuBot className="text-primaryColor" /> AI Assistant
        </h2>
        {isStreaming && (
          <button
            onClick={() => abortControllerRef.current?.abort()}
            className="text-xs bg-red-50 text-red-600 px-3 py-1.5 rounded-full hover:bg-red-100 transition-colors font-medium border border-red-200"
          >
            Stop Generating
          </button>
        )}
...
const ChatInput = ({ onSend, disabled }: any) => {
  ...
  return (
    <form
      className="p-4 bg-white dark:bg-slate-900  dark:border-slate-800 flex gap-2 items-end"
...
const WelcomeHero = () => (
  <div className="flex flex-col items-center justify-center h-full opacity-60 pointer-events-none select-none">
    ...
    <p className="text-sm text-slate-500 dark:text-slate-400 mt-2">
      Ask me anything about your data or code.
    </p>
  </div>
);
